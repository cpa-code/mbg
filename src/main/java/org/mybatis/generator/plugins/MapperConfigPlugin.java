 ...
 * <li><code>entityTargetPackage</code> (required) the name of the package where the file
 * should be placed. Specified like "com.mycompany.sql".</li>
 * <li><code>entityTargetProject</code> (required) the name of the project where the file
 * should be placed.</li>
 * </ul>
 * 
 * <p>Note: entityTargetPackage and entityTargetProject follow the same rules as the
 * entityTargetPackage and entityTargetProject values on the sqlMapGenerator configuration
 * element.
 * 
 * @author Jeff Butler
 * 
...
public class MapperConfigPlugin extends PluginAdapter {
    ...
    public boolean validate(List<String> warnings) {
        ...
        if (!stringHasValue(properties
                .getProperty("entityTargetProject"))) { //$NON-NLS-1$
            warnings.add(getString("ValidationError.18", //$NON-NLS-1$
                    "MapperConfigPlugin", //$NON-NLS-1$
                    "entityTargetProject")); //$NON-NLS-1$
            valid = false;
        }

        if (!stringHasValue(properties
                .getProperty("entityTargetPackage"))) { //$NON-NLS-1$
            warnings.add(getString("ValidationError.18", //$NON-NLS-1$
                    "MapperConfigPlugin", //$NON-NLS-1$
                    "entityTargetPackage")); //$NON-NLS-1$
            valid = false;
        }
    ...
    public List<GeneratedXmlFile> contextGenerateAdditionalXmlFiles() {
        ...
        GeneratedXmlFile gxf = new GeneratedXmlFile(document, properties
                ...
                properties.getProperty("entityTargetPackage"), //$NON-NLS-1$
                properties.getProperty("entityTargetProject"), //$NON-NLS-1$
